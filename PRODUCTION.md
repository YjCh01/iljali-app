...
2. **기본 플랜** — 공고 등록 무료 · 근무지 700m 지도 핀 무료 · 가입 보너스 2회·인증 보너스 5회 · 지원·채팅 무료 · **채용 성공 수수료 없음**
3. **유료 상품** — 일자리 알림핀·정류장 표시핀·PUSH 이용권 각 **19,900원** (700m) · 10회 팩 10% 할인 · 보너스 30일 / 유료 180일
4. **공고 등록** — 제휴사 여부 → 근로자 유형(일반/일용직/계약직) → 일자리 작성(근무일자 달력·급여 단위)
5. **푸시 소진** — 가입 보너스 → 인증 보너스 → 패키지 크레딧 순 · 패키지 구매는 PaymentFlowHelper → WebView
6. **채용 성공 수수료** — 메인 앱 **없음** (`ENABLE_HIRING_COMMISSION` 기본 false). 제휴 채널 빌드만 `commission_calculator` 결제 플로우 활성
