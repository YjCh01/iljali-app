...
- **도로 추종**: 서버 `route_geometry_service` (OSRM driving) → 노선 upsert 시 `polylinePoints` densify · `POST .../refresh-geometry`
- **클라이언트**: `ShuttleRouteVisibility`가 밀집 polyline을 정류장 직선으로 덮어쓰지 않음 · upsert가 서버 응답 반영 · 방향 화살표를 densified 세그먼트 기준
- 테스트: pytest `test_route_geometry` 4 pass · flutter visibility 4 pass
- 기존 노선은 재저장 또는 refresh-geometry 후 도로선 적용

## 2026-07-13 — 지도보기: 핀 중앙 + 콜아웃은 탭 시에만

- 요구: 공고「지도보기」→ 근무지핀 화면 중앙, 그리드(콜아웃) 자동 표시 금지
- 콜아웃은 근무지/알림/정류장 핀 클릭·터치 시에만
- `_applyFocusPostId`에서 `onSelectedPinChanged` 제거, `pinScreenY: 0.5`
- 핀 탭 시에만 콜아웃 + `calloutPinScreenY` 카메라 오프셋

## 2026-07-13 — 지도 4이슈 재수정 (재배포 필수)

- 사용자: 재배포 후에도 알림핀 미표시·콜아웃 가림·새로고침 숨김·근무지색 오염
- 원인: 이전 수정이 부분만 반영/알림핀 소스가 displayPins만·콜아웃 하단 앵커가 핀 가림·새로고침 `_areaSearchPending` 게이트
- 수정:
  1) 새로고침 버튼 항상 표시
  2) 콜아웃을 핀 **위**(top) + 핀을 Y=0.62로 내림
  3) 알림핀: `_ownPosts`/`focusPost` 병합 + 기업지도는 잠금 없이 설정 핀 전부 표시
  4) 근무지 마커 `packageActive` → 강제 회색
- **실서비스 웹/앱 재배포 후** 하드 리프레시 필요 (이전 배포본에는 불완전)

## 2026-07-13 — TestFlight 업로드 성공 (필드 혼동 + Fastfile 수정)

- 사용자가 이메일 칸에 앱전용비번(`nzeo-…`)을 넣음 → 스크립트 자동 교정 + 순서 안내 강화
- Fastfile: `FASTLANE_PASSWORD`에 앱전용비번 넣지 않음 + `apple_id: 6789877583`
- **map.ipa TestFlight 업로드 OK** (처리 5~15분)

## 2026-07-13 — TestFlight 비밀번호 갱신 쉽게

- `도구_TestFlight비밀번호갱신.command` — 브라우저 열고, 앱전용비번만 붙여넣기, 선택 업로드
- `scripts/refresh_testflight_password.sh` — 로그인 비번 ≠ 앱전용비번 안내
- 업로드 실패 시 `도구_TestFlight업로드만.command` 가 갱신 도구로 안내

## 2026-07-13 — 알림핀 미표시 + 근무지핀 색 오염 (치명)

- 증상: test1 알림핀 3개 설정 후 지도에 근무지핀만 보임, 근무지색이 마지막 알림핀 색(보라)처럼 보임
- 원인:
  1) 기업 홈 지도가 `JobRecruitmentMapPin`을 아예 안 그림
  2) sync hydrate가 `basePoints[1+]` 전부 폐기 (근무지만 재생성)
  3) `packageActive`/지갑 크레딧이 근무지 핀 색을 보라로 올림
- 수정:
  - 기업 지도에 알림핀+연결선 렌더 (`pinColorHex` 반영)
  - `notification_settings_json` 서버 영속 + bootstrap hydrate
  - 근무지 색 = 시급만 (회색/하늘), packageActive 무시
  - 옵션 시트 persist 시 `pushPostUpdate`로 서버 동기화
- 테스트: recruitment factory / tier resolver 15 pass

## 2026-07-13 — 핀 콜아웃이 핀을 덮던 문제

- 원인: corporate_home_tab 콜아웃이 `top: height*0.38` 고정 → 지도 중앙 핀과 겹침
- 수정: 드래그 시트 바로 위 `bottom` 앵커 (`MapFloatingInsets.pinCalloutAboveSheet`) + maxHeight 36% 스크롤
- 카메라: `focusPin(pinScreenY: 0.34)` — native pivot / web lat bias로 핀을 중상단에
- 테스트: map_floating_insets_test 3 pass
- 확인: 실서비스 웹 재배포 후 Cmd+Shift+R

## 2026-07-13 — 지도보기 핀 누락 + 현지도검색 아이콘화

- 지도보기: focus 핀을 `_catalogPins`에 upsert — 콜아웃만 뜨고 근무지핀 없던 버그 수정
- 「현 지도에서 검색」넓은 필 → 현재위치 아래 **새로고침 아이콘** (기업/구직자/mock)

## 2026-07-13 — 근무일정 협의 시 급여지급일 검증 버그

- 원인: 일용직 급여일을 근무일+1로만 계산 → 협의(근무일 없음)면 `null` → 등록 차단
- 수정: `buildSalaryPaymentSchedule` — 근무일정 협의면 급여일도 **협의** 처리 (작성/수정 폼)

## 2026-07-13 — 웹 핀 SVG를 Canvas와 동기화 (긴급)

- 원인: 네이티브 Canvas는 새 핀, **웹 `pinHtml`은 옛 링 물방울** → 실서비스 웹에 새 핀 미반영
- 수정: workplace plain · notification flag · busStop 사각+아이콘 + `ringTint` 웹 전달
- `JobMapWebMarkerFactory` 유료 ringTint; 캐시 `pin_v6_`
- 웹 빌드 OK (`build/web-deploy/site`) — **NCP SSH 키 거부**로 업로드 대기 (비밀번호 배포 필요)
- 서버 job_posts **0건** — 새 공고가 로컬만일 수 있음 (`JobPostSync` 실패 로그 추가)

## 2026-07-13 — 지도 테스트 데이터 전량 삭제

- 프로덕션: 유령핀 **265** · 유령노선 **1** · 공고 **1** · 셔틀노선 **1** · 고아 지원 **1** 삭제
- 추가: `purge_map_content` 서비스/어드민 API · `scripts/purge_map_content_api.sh` / `_server.sh`
- 확인: jobs/ghosts/apps/routes 모두 **0** (회원·지갑 유지)

## 2026-07-13 — 유료 핀 링 ringTint(bodyColor)

- `MapPinColors.ringTint` 추가 (HSL lightness +0.28)
- `JobMapMarkerFactory` 유료 ring: `packagePurpleRing` → `ringTint(bodyColor)`
- 검증 페이지 유료 핀도 동일

## 2026-07-13 — 셔틀 방향 화살표 20px

- verify/factory 화살표 **25 → 20px** (캐시키 `shuttle_arrow_v4_…`)

## 2026-07-13 — 셔틀 방향 화살표 25px

- verify/factory 화살표 **50 → 25px** (캐시키 `shuttle_arrow_v3_…`)

## 2026-07-13 — 셔틀 방향 화살표 50px 확정

- 원인: `shouldRepaint => false` + 화살표를 핀 아래에 그려 hot reload/체감 변화 없음
- 수정: verify/factory 모두 **50px**, 흰 테두리, 핀 **위**에 그림; 캐시키 `shuttle_arrow_v2_…`
- 검증: `http://localhost:7358` 핀 검증 페이지에서 50px 화살표 확인

## 2026-07-13 — 셔틀 점선 노선 방향 화살표

- `ShuttleRouteOverlayFactory` — 정류장 구간마다 진행방향 NMarker 화살표 (노선색, 14px)

## 2026-07-12 — 핀 도안 재정렬 (plain + flag)

- workplace → `_paintPlainTeardrop` (구멍 없음), 유료만 packagePurpleRing stroke
- notification → `_paintFlagTeardrop` (우상단 깃발)
- JobMapMarkerFactory 유료 ringWidth 3; 검증 스크린샷 docs/pin-visual-verify.png

## 2026-07-12 — 핀 색/모양 버그 수정 + 검증 스크린샷

- MapPinColors freeGray/packagePurple/packagePurpleRing
- pinColor·pinBorderColor 티어 분기; JobMapMarkerFactory bodyColor + `_styleFor`→workplace 고정
- 웹 JobMapWebMarkerFactory도 workplace 고정; 알림·정류장 아이콘 packagePurple
- 검증: `lib/main_pin_verify.dart` + `PinVisualVerifyPage` → `docs/pin-visual-verify.png`

## 2026-07-12 — 핀 색 티어별 분기 (실제 적용)

- `MapPinColors` — freeGray / packagePurple / packagePurpleRing 추가
- `JobMapPinDisplayTierX.pinColor·pinBorderColor` — standard/고시급/유료/유령 분리
- `JobMapMarkerFactory` + 웹 `JobMapWebMarkerFactory` — `displayTier.pinColor` 사용
- 알림핀(recruitment) 아이콘 — packagePurple

## 2026-07-12 — 핀 디자인 다시잡기 (토큰 + 위젯)

- `lib/theme/app_colors.dart` — purple/freeGray 핀 팔레트 (지시 그대로)
- `lib/widgets/map/pins.dart` — WorkplacePin / JobAlertPin / StopPin / ClusterBadge
- TEMP: `MapApp` 기동 직후 WorkplacePin(isPaid:true) ×3 — 로그인 게이트 우회 (hot restart)

## 2026-07-10 — 기업 홈 지도 강남 고정 버그

- 원인: 세션 뷰포트 캐시·네이티브 `saved==null → 강남` 분기가 사업소재지보다 우선
- 수정: 사업소재지 → 자사 공고 근무지 폴백, 정책 중심 강제 이동, 로그아웃 시 캐시 삭제

## 2026-07-10 — 배포 빌드 실패 수정 (CorporateJobPost import)

- `push_ticket_use_page.dart` — 지도 정책 작업 중 import 누락으로 web/Android 컴파일 실패 → 복구

## 2026-07-10 — 지도 초기 위치 정책 (MapInitialCenterPolicy)

- 기업: 사업소재지 · 개인: 실주소 · 공고 행위: 근무지→사업소재지
- `.cursor/rules/map-initial-center.mdc` — 재발 방지

## 2026-07-10 — 기업 지도 초기 위치 → 근무지 우선

- `WorkplaceMapCenterResolver` — 주소 지오코딩 후 지도 중심
- `PushRadiusMapPicker` — 강남 뷰포트 캐시가 근무지와 다르면 무시
- 셔틀 노선 등록 — 공고·사업장 근무지 좌표 전달 + 자동 정렬

## 2026-07-10 — TestFlight 업로드 자동화

- `Gemfile` + `fastlane/Fastfile` — `prepare_signing` · `beta` · `flutter build ipa` + `upload_to_testflight`
- `scripts/setup_testflight.sh` · `scripts/upload_testflight.sh` · `도구_TestFlight설정/업로드.command`
- `fastlane/.env.example` — APPLE_TEAM_ID + ASC API Key 또는 FASTLANE_USER

## 2026-07-10 — iOS 빌드 Sentry 호환 수정

- `sentry_flutter` 8.14.2 → 9.24.0 (`SentryBinaryImageCache` Swift 컴파일 오류 해결)
- `ios/Podfile`: `use_frameworks! :linkage => :static`, `platform :ios, '13.0'`
- `flutter build ios --release --no-codesign` 성공 확인

## 2026-07-09 — 구직자 이력서 AI 불러오기

- **기능**: 알바몬·잡코리아 링크 / PDF·캡처 / 텍스트 붙여넣기 → 학력·경력·면허·자격증·자기소개 추출
- **API**: `POST /v1/resume-import/parse`, `POST /v1/resume-import/parse-file`
- **UI**: 내 이력서 · 이력서 작성 허브 → 「내 이력서 AI로 불러오기」
- **한계**: 로그인 필요 URL은 캡처·PDF 권장 (CLOVA OCR·pypdf)

## 2026-07-08 — 어드민 셔틀 노선 엑셀 일괄 등록

- **양식**: `assets/templates/shuttle_route_import_template.xlsx` — 단일 시트(노선입력), 노선명·정류장순서·정류장명·도착시간·주소
- **규칙**: 같은 노선명 = 1노선, 마지막 순서 = 근무지 도착, 경유 정류장 = 탑승 시각(HH:MM)
- **API**: `POST /v1/admin/ops/shuttle/routes/bulk-import` (multipart, admin key)
- **UI**: 어드민 → 회원 → 기업 선택 → 「셔틀 노선 엑셀 일괄 등록」 카드
...
## 2026-07-12 — 약관 확정 + TestFlight iPhone OK

- **약관 11종** 시행 2026-09-01, REVIEW 제거, 19,900원 분리, 30/180일, 토스/NCP
- **TestFlight Build 4** iPhone 설치·동작 확인
- **LBS** 사업계획서 초안 `store/legal/lbs_business_plan_iljari.md`
- **아침 브리핑** `docs/MORNING_BRIEF_2026-07-12.md`

## 2026-07-11 — TestFlight ITMS-90683 (Info.plist privacy)

- **원인**: Build 2 Apple 거절 — `NSCameraUsageDescription`, `NSPhotoLibraryUsageDescription` 누락 (SDK 참조)
- **수정**: `ios/Runner/Info.plist` 카메라·앨범·위치(Always) purpose string 추가
- **빌드**: `1.0.0+4` → 재업로드 필요

## 2026-07-11 — TestFlight 업로드 성공

- **IPA**: `map.ipa` 1.0.0 (2) → App Store Connect App `6789877583`
- **인증**: Apple ID + 앱 전용 비밀번호 (`fastlane/.env`)
- **스크립트**: `--upload-only`, `도구_TestFlight업로드만.command`, Fastfile `app_specific_password` env 수정
- **다음**: ASC 빌드 처리 대기 → 내부 테스터 → `도구_실서비스한방배포` (웹·API)

## 2026-07-11 — Xcode·배포 오류 메시지 정리

- **원인**: Apple Developer 승인 후에도 스크립트가 「승인 대기」 안내; `fastlane/.env` 템플릿만 있고 값 비어 있음 → 업로드 단계만 실패.
- **IPA 빌드**: `flutter build ipa` 성공 (team JR36AGC7R9, `build/ios/ipa/*.ipa`).
- **수정**: `scripts/iljari_ios_env.sh` (Team Xcode 자동감지), `preflight_deploy.sh`, `도구_배포점검.command`, setup/upload/build/deploy 메시지 정리, Fastfile Automatic Signing.

## 2026-07-11 — YOLO 출시 총괄 점검 (Founder Return)

- **로드맵**: `docs/FOUNDER_RETURN_STATUS.md` — 기술 6단계(`LAUNCH_ROADMAP.md`) + Pre-Toss 6단계 통합 재정리.
- **실서비스**: `api.iljari.app/health` ok, `free_exposure_promo: true`, `toss_configured: false`.
- **버그**: `map_exposure_visual_policy` shuttle_route_visibility import 경로 수정.
- **서버 테스트**: `conftest.py` autouse `create_all`; admin 기업가입 휴대폰인증; pilot seeker display_name — **71 passed**.
- **어드민**: 파일럿 패널 `AdminShuttleParticipantsCard` — 노선공유·관제탑·정류장 선택 현황.
- **배포 권장**: 맵 정책·빌드 fix 반영을 위해 `도구_실서비스한방배포` 1회.
